...
#include "slgw_app.h"
#include "slgw_text.h"
#include "../../driver/smartlight_ioctl.h"
...
static void slgw_driver_close_fd(slgw_driver_transport *transport, const char *reason) {
    int fd = transport->fd;
    transport->fd = -1;
    ...
    slgw_app_notify_transport_state(transport->app, false, reason == NULL ? "closed" : reason);
}

static int slgw_driver_open_fd(slgw_driver_transport *transport) {
    int fd = open(transport->device, O_RDWR | O_NONBLOCK);
    if (fd < 0) {
        return -1;
    }
    ioctl(fd, SL_IOC_CLEAR_BUFFERS, 0);
    transport->fd = fd;
...
static void *slgw_driver_reader_main(void *arg) {
    slgw_driver_transport *transport = (slgw_driver_transport *)arg;
    ...
    while (!transport->stop_requested) {
        ...
        if (transport->fd < 0) {
            if (slgw_driver_open_fd(transport) != 0) {
        ...
        if (rc < 0) {
            slgw_driver_close_fd(transport, "select_failed");
            slgw_sleep_ms(transport->reconnect_delay_ms);
            continue;
        }
        if (rc == 0) {
        ...
        if (FD_ISSET(fd, &readfds)) {
            ...
            ssize_t idx;
            if (nread <= 0) {
                slgw_driver_close_fd(transport, nread == 0 ? "read_eof" : "read_failed");
                slgw_sleep_ms(transport->reconnect_delay_ms);
                continue;
            }
...
int slgw_driver_transport_init(slgw_driver_transport *transport, struct slgw_app *app, const slgw_config *cfg) {
    ...
    if (slgw_copy_text(transport->device, sizeof(transport->device), cfg->driver_device) != 0) {
        return -1;
    }
...
int slgw_driver_transport_start(slgw_driver_transport *transport) {
    ...
    return pthread_create(&transport->reader_thread, NULL, slgw_driver_reader_main, transport);
}
...
void slgw_driver_transport_stop(slgw_driver_transport *transport) {
    ...
    if (transport->fd >= 0) {
        slgw_driver_close_fd(transport, "stopped");
    }
...
int slgw_driver_transport_write_line(slgw_driver_transport *transport, const char *line) {
    ...
    if (nwritten < 0) {
        slgw_driver_close_fd(transport, "write_failed");
        return -1;
    }
    return 0;
}
