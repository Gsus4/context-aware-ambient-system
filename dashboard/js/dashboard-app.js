...
const state={uiState:{controlMode:'scene',emotionMode:'work',smartModeEnabled:true},sensorData:{heartRate:null,spo2:null,aqi:null,temperature:null,humidity:null,comfort:null,pirDetected:null,pirTime:null,onlineDevices:0,totalDevices:3,systemStatus:'等待實機資料',moduleStatus:null,deviceConnectionText:''},deviceState:{ledOn:null,brightness:null,kelvin:null,ledColor:'#2d3748',fanOn:null,fanSpeed:null,breathingEnabled:false,breathingSpeed:'slow',breathingStrength:'medium',flowEnabled:false,flowPreset:'soft_rainbow',flowSpeed:'slow',flowBrightness:null},audioState:{connected:null,playing:false,mode:null,volume:null,track:null,progressSec:null,durationSec:null,trackIndex:0},trends:{heart:[],spo2:[],aqi:[],temp:[],humidity:[]},weeklyTrends:{heart:[],spo2:[],aqi:[],temp:[],humidity:[]},events:[]};
...
function moduleAlias(key){if(['heart','spo2','wearable'].includes(key))return 'wearable';if(['aqi','tempHumidity','pir','fan','env','envComfort'].includes(key))return 'envComfort';return key}
...
function applyModuleControlLocks(modules){
  ...
  setControlLocked($('#fanRange'),!fanOnline||smartLocked);
  setManyLocked('.audio-btn, #audioProgressRange, #audioVolumeRange',!audioOnline||smartLocked);
...
function render(){
  ...
  $('#summaryComfort').textContent=nval(s.comfort);
...
function renderAudioState(){
  ...
  randomizeAudioWave();
  if(track)track.textContent=textOrDash(a.track);
...
function getCameraSourceUrl(){
  return state.sensorData.cameraSnapshotUrl||window.DASHBOARD_CONFIG?.cameraSnapshotUrl||window.DASHBOARD_CONFIG?.camera?.snapshotUrl||window.DASHBOARD_CONFIG?.camera?.streamUrl||'';
}
...
function bind(){document.addEventListener('click',e=>{if(e.target.closest('.module-control-locked,.disabled,[disabled]'))return;const m=e.target.closest('.mode-card');if(m){const nextControlMode=m.dataset.controlMode;state.uiState.controlMode=nextControlMode;if(nextControlMode==='scene'||nextControlMode==='manual'){state.uiState.smartModeEnabled=false;}addEvent(`<strong>控制模式：</strong>${controlModeName(nextControlMode)}`,controlModeIcon(nextControlMode),controlModeAccent(nextControlMode));render()}const sm=e.target.closest('.small-mode');if(sm){const nextMode=sm.dataset.mode;state.uiState.controlMode='scene';state.uiState.smartModeEnabled=false;state.uiState.emotionMode=nextMode;if(window.DashboardSceneMemory?.applySceneProfile){window.DashboardSceneMemory.applySceneProfile(nextMode,true);addEvent(`<strong>切換情境模式：</strong>${modes[nextMode]?.name||'情境模式'}`,'relax','var(--purple)')}else{updateMode(nextMode,true,{silent:true})}}const sw=e.target.closest('.switch');if(sw){if(sw.dataset.toggle==='smart-mode'){const enabled=state.uiState.smartModeEnabled===false;if(enabled&&state.uiState.controlMode==='manual'){state.uiState.controlMode='scene';}updateUiState({smartModeEnabled:enabled});addEvent(enabled?'<strong>智慧模式：</strong>已開啟':'<strong>智慧模式：</strong>已關閉，保留自定義設定','bot',enabled?'var(--green)':'var(--yellow)');return}if(sw.dataset.toggle==='led'){updateDeviceState({ledOn:!state.deviceState.ledOn})}}const ab=e.target.closest('[data-audio-action]');if(ab){audioAction(ab.dataset.audioAction);return}const cd=e.target.closest('.color-dot');if(cd){updateDeviceState({ledColor:cd.dataset.color,ledOn:true})}});$('#brightnessRange')?.addEventListener('input',e=>{const value=clampSceneAdjustment('brightness',e.target.value);e.target.value=value;updateDeviceState({brightness:value})});$('#brightnessRange')?.addEventListener('change',e=>{const value=clampSceneAdjustment('brightness',e.target.value);e.target.value=value;updateDeviceState({brightness:value})});$('#kelvinRange')?.addEventListener('input',e=>{const value=clampSceneAdjustment('kelvin',e.target.value);e.target.value=value;updateDeviceState({kelvin:value})});$('#kelvinRange')?.addEventListener('change',e=>{const value=clampSceneAdjustment('kelvin',e.target.value);e.target.value=value;updateDeviceState({kelvin:value})});$('#fanRange')?.addEventListener('input',e=>{const level=clamp(Math.round(+e.target.value),0,10);updateDeviceState({fanSpeed:level,fanOn:level>0})});$('#fanRange')?.addEventListener('change',()=>{if(!state.deviceState.fanOn)return;addEvent(`<strong>風扇等級調整為</strong> ${fanState(state.deviceState.fanSpeed)}（${Math.round(state.deviceState.fanSpeed)}級）`,'fan','var(--cyan)')});const audioVolumeRange=$('#audioVolumeRange');if(audioVolumeRange){audioVolumeRange.addEventListener('input',e=>{state.audioState.volume=clamp(+e.target.value,0,100);renderAudioState()});audioVolumeRange.addEventListener('change',()=>renderAudioState())}const audioProgressRange=$('#audioProgressRange');if(audioProgressRange){audioProgressRange.addEventListener('input',e=>{const dur=Math.max(1,Number(state.audioState.durationSec)||1);state.audioState.progressSec=clamp(+e.target.value,0,dur);renderAudioState()});audioProgressRange.addEventListener('change',()=>renderAudioState())}$('#applyBtn')?.addEventListener('click',()=>{if(window.DashboardSceneMemory?.saveCurrentScene?.())return;toast('已套用目前燈光設定。')});$('#resetBtn')?.addEventListener('click',()=>{if(window.DashboardSceneMemory?.resetCurrentModeDefault?.())return;toast('已恢復目前模式預設。')});$('#applyRecommendBtn')?.addEventListener('click',()=>{updateMode(state.uiState.emotionMode,true,{silent:true});toast(`已套用推薦模式：${modes[state.uiState.emotionMode].name}`)});const openCameraBtn=$('#openCameraBtn');if(openCameraBtn)openCameraBtn.addEventListener('click',openCameraModal);['#closeCameraBtn','#closeCameraBtn2'].forEach(sel=>{const btn=$(sel);if(btn)btn.addEventListener('click',closeCameraModal)});const refreshCameraBtn=$('#refreshCameraBtn');if(refreshCameraBtn)refreshCameraBtn.addEventListener('click',openCameraModal);const cameraModal=$('#cameraModal');if(cameraModal)cameraModal.addEventListener('click',e=>{if(e.target.id==='cameraModal')closeCameraModal()});$('#openTrendHistoryBtn')?.addEventListener('click',openTrendHistoryModal);$('#closeTrendHistoryBtn')?.addEventListener('click',closeTrendHistoryModal);$('#openEventHistoryBtn')?.addEventListener('click',openEventHistoryModal);$('#closeEventHistoryBtn')?.addEventListener('click',closeEventHistoryModal);const trendModal=$('#trendHistoryModal');if(trendModal)trendModal.addEventListener('click',e=>{if(e.target.id==='trendHistoryModal')closeTrendHistoryModal()});const eventModal=$('#eventHistoryModal');if(eventModal)eventModal.addEventListener('click',e=>{if(e.target.id==='eventHistoryModal')closeEventHistoryModal()});document.addEventListener('keydown',e=>{if(e.key==='Escape'){closeTrendHistoryModal();closeEventHistoryModal();closeCameraModal()}});addEventListener("resize",()=>{resizeDashboard();scheduleDrawAll();if($('#trendHistoryModal')?.classList.contains('show'))renderTrendHistoryModal()})}
