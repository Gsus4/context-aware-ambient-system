/*
  ...
  實機版 mqtt：
  - 前端透過 WebSocket / REST 將操作交給 Bridge。
  - Bridge 再轉 MQTT 給 SmartLight NODE。
...
(function () {
  ...
  let lastConnectionState = isDevMode
    ? { websocket: false, mqtt: false, smartLight: 'mock', onlineDevices: 0, totalDevices: 3, moduleStatus: [] }
    : { websocket: false, mqtt: false, smartLight: 'unknown', onlineDevices: 0, totalDevices: 3, moduleStatus: [] };
  let debugLogEntries = [];
  ...
  function canSendCommand(action) {
    ...
    if (action === 'set_fan_level' || action === 'set_fan_mode') return moduleOnline('fan') && !smartModeEnabled();
  ...
  function deriveConnectionSensorData(connectionState) {
    ...
    const total = Number(connectionState.totalDevices || (moduleStatus ? 3 : 3));
  ...
  function applyDashboardPayload(payload) {
    ...
    // 原始 MQTT / Bridge 除錯資訊只進 Debug Log。
    ...
    if (payload.mode) {
      ...
      if (currentMode !== payload.mode) {
        // Bridge / NODE 若只回傳模式但亮度、色溫仍是上一個情境，
  ...
  function startWebSocket(wsConfig) {
    ...
    ws.addEventListener('open', () => {
      ...
      applySourceStatus('Bridge 已連線，等待 SmartLight 狀態');
      addUserEvent({ text: '<strong>Dashboard：</strong>已連上 Bridge', icon: 'device', accent: 'var(--green)' });
      markActive();
      sendVisibility();
      sendDashboardCommand('get_status', {}, { silent: true, allowWhenOffline: true });
    });
    ...
    ws.addEventListener('close', () => {
      ...
      applySourceStatus('Bridge 已斷線');
      addUserEvent({ text: '<strong>Dashboard：</strong>Bridge 已斷線', icon: 'alert', accent: 'var(--yellow)' });
    });

    ws.addEventListener('error', (error) => {
      console.error('[Dashboard Adapter] WebSocket 錯誤：', error);
      applySourceStatus('Bridge 連線錯誤');
  ...
  async function sendDashboardCommand(action, data = {}, options = {}) {
    ...
    try {
      ...
      addUserEvent({
        text: `<strong>指令送出失敗：</strong>${error.message || '請確認 Bridge 是否啟動'}`,
        icon: 'alert',
        accent: 'var(--yellow)'
      });
    }
  ...
  const sendSceneModeCommand = debounce((payload) => {
    sendDashboardCommand('set_mode', payload);
  }, 300);
  ...
  function bindSmartLightUiCommands() {
    ...
    document.addEventListener('click', (event) => {
      ...
      if (controlModeButton) {
        ...
        if (isDevMode) {
          ...
          // 先通知 Bridge 關閉智慧模式，避免後端 state→LED 測試又把燈拉回自動 / 測試燈號。
      ...
      if (applyRecommendButton) {
        setTimeout(() => {
          const light = getCurrentLightState();
          sendSceneModeCommand({ mode: light.mode });
        }, 0);
        return;
      }
    ...
    if (fanRange) {
      ...
      fanRange.addEventListener('change', () => {
        if (fanRange.disabled || smartModeEnabled()) return;
